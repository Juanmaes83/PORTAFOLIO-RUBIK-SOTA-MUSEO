...
function Artwork({ project, focused }: { project: MuseumProject; focused: boolean }) {
  ...
  const inward = project.side === "left" ? 0.08 : -0.08;
  ...
  return (
    <group position={[x, 1.75, project.z]} rotation={[0, rotationY, 0]}>
      ...
      <mesh position={[0, 0, 0.09 + inward * 0]}>
...
export default function MuseumScene({ projects, focusedId }: Props) {
  ...
  return (
    <>
      ...
      <mesh position={[0, -0.08, 0]} receiveShadow material={floorMaterial}>
        <boxGeometry args={[9.5, 0.16, 31]} />
      </mesh>

      <mesh position={[-4.85, 2.55, 0]} receiveShadow>
        <boxGeometry args={[0.22, 5.1, 31]} />
      ...
      <mesh position={[0, 5.02, 0]} receiveShadow>
        <boxGeometry args={[9.7, 0.18, 31]} />
        <meshStandardMaterial color="#181714" roughness={0.92} />
      </mesh>

      <mesh position={[0, 2.55, -15.1]}>
        <boxGeometry args={[9.5, 5.1, 0.28]} />
        <meshStandardMaterial color="#ded6ca" roughness={0.9} />
      </mesh>
      <mesh position={[0, 2.55, 14.2]}>
        <boxGeometry args={[9.5, 5.1, 0.28]} />
        <meshStandardMaterial color="#d9d1c4" roughness={0.9} />
      </mesh>

      <group position={[0, 0.42, 1.1]}>
        <mesh castShadow receiveShadow>
          <boxGeometry args={[2.1, 0.42, 0.72]} />
          <meshStandardMaterial color="#54483d" roughness={0.72} />
        </mesh>
        <mesh position={[-0.78, -0.37, 0]}>
          <boxGeometry args={[0.12, 0.72, 0.58]} />
          <meshStandardMaterial color="#26221e" roughness={0.82} />
        </mesh>
        <mesh position={[0.78, -0.37, 0]}>
          <boxGeometry args={[0.12, 0.72, 0.58]} />
          <meshStandardMaterial color="#26221e" roughness={0.82} />
      ...
      <group position={[0, 2.0, -14.82]}>
        <mesh>
          <boxGeometry args={[3.4, 2.4, 0.08]} />
          <meshStandardMaterial color="#26231f" roughness={0.68} />
        </mesh>
        <Text position={[0, 0.08, 0.08]} fontSize={0.38} color="#e7d8c5" anchorX="center" anchorY="middle">
          RUBIK SOTA
        </Text>
        <Text position={[0, -0.56, 0.08]} fontSize={0.14} color="#b9aa98" anchorX="center" anchorY="middle">
